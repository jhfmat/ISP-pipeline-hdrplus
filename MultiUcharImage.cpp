...
bool MultiUcharImage::GetHistogram(int pHistB[], int pHistG[], int pHistR[])
{
	int x, y, g;
...
bool MultiUcharImage::ResizeImage(MultiUcharImage *pOutImage, int OutnWidth, int OutnHeight)
{
	int nInWidth = GetImageWidth();
	int nInHeight = GetImageHeight();
...
bool MultiUcharImage::RGB2Y(SingleUcharImage *pOutImageY)
{
	int nWidth = GetImageWidth();
	int nHeight = GetImageHeight();
	if (pOutImageY->GetImageWidth() != nWidth || pOutImageY->GetImageHeight() != nHeight)
	{
		if (!pOutImageY->CreateImage(nWidth, nHeight))return false;
	}
	int nThread = omp_get_num_procs();
#pragma omp parallel for num_threads(nThread) schedule(dynamic,32)
	for (int y = 0; y < m_nHeight; y++)
	{
		unsigned char *pLine = GetImageLine(y);
		unsigned char *pOutLine = pOutImageY->GetImageLine(y);
		int x = 0;
		for (; x < m_nWidth; x++, pLine += 3)
		{
			int  yuv[3], bgr[3];
