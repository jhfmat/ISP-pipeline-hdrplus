...
bool Yuv420Image::YUV444ToYUV420OPT(MultiUcharImage *pInputYUV444Image)
{
	for (int y = 0; y < m_nHeight;y++)
	{
		unsigned char *prgbline = pInputYUV444Image->GetImageLine(y);
		for (int x = 0; x < m_nWidth; x++)
		{
			unsigned char tmp=prgbline[0];
			prgbline[0] = prgbline[2];
			prgbline[2] = tmp;
			prgbline += 3;
		}
	}
...
#pragma omp parallel for num_threads(nThread) firstprivate(loop) private(pUVLines) 
		for(int y=1; y<m_nHeight-1; y+=2)
		{
			if (loop==0)
			{
				...
				HIntUVLine(GetUVLine(y), pUVLines[0], m_nWidth);
				loop++;
			}
			int  UV[2];
			HIntUVLine(GetUVLine(y + 1), pUVLines[1], m_nWidth);
...
void Yuv420Image::HIntUVLine(unsigned char *pInLine, unsigned short *pOutLine, int nWidth)
{
