...
public:
	...
	bool ConfigParamListAddVariable(char *pParamName, int *pParamAddr, int nMin=0, int nMax=0x7fffffff)
	{
	...
	bool ConfigParamListAddArray(char *pParamName, int pVector[], int nCh, int nMin = 0, int nMax = 0x7fffffff)
	{
		char ParamName[64];
		for (int i = 0; i < nCh; i++)
		{
			sprintf(ParamName, "%s_%d", pParamName, i);
			if (!ConfigParamListAddVariable(ParamName, pVector + i, nMin, nMax))return false;
		}
		return true;
	}
	bool ConfigParamListAddMatrixRow(char *pParamName, int *pMatrixRow, int nRow, int nCol, int nMin = 0, int nMax = 0x7fffffff)
	{
		char ParamName[64];
		sprintf(ParamName, "%s_%d", pParamName, nRow);
		if (!ConfigParamListAddArray(ParamName, pMatrixRow, nCol, nMin, nMax))return false;
		return true;
	}
	...
	bool SaveParamListToFile(FILE *fp)
	{
		...
		for (int i = 0; i < m_nConfigParamListCount; i++)
		{
			...
			int res = fprintf(fp, "%s=%d;\tValueRange=[%d,%d]\n", pParamItem->m_pConfigParamName, ParamValue, pParamItem->m_nConfigParamMin, pParamItem->m_nConfigParamMax);
			pParamItem = pParamItem->m_pConfigParamNext;
		}
		return true;
	}
